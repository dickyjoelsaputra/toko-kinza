                    ...
                    <div class="row mx-2">
                        ...
                        <div class="col-md-6 col-12">
                            <label class="custom-switch p-0 mt-md-2 mt-3">
                                <input type="checkbox" id="manualSwitch" class="custom-switch-input">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.print-btn', function() {
                ...
                'data-name');
                var barcodeContainer = $('<svg>').attr('id', 'barcode-container');
                var barcodeOptions = {
                    format: 'CODE128',
                    lineColor: '#000',
                    width: 2,
                ...
                JsBarcode(barcodeContainer[0], itemCode, barcodeOptions);

                var barcodeSVG = barcodeContainer[0].outerHTML;

                var printWindow = window.open('', '_blank');
                printWindow.document.write('<html><head><title>Barcode</title>');
                printWindow.document.write('<style>@page { size: 58mm auto; }</style>');
                printWindow.document.write('<style>hr { border-top: 1px dashed #000; }</style>');
                printWindow.document.write('</head><body>');
                printWindow.document.write('<div style="text-align: center;">' + itemName + '</div>');
                printWindow.document.write('<div style="text-align: center;">' + barcodeSVG + '</div>');
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                var newTab = window.open(editUrl, '_blank');
                newTab.focus();
            });
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $.each(items, function(index, item) {
                            ...
                            actionButtons.append($("<button>").addClass(
                                "btn btn-primary edit-btn mr-1").attr("data-id", item
                                .id).text("Edit"));
                            actionButtons.append($("<button>").addClass(
                                "btn btn-warning print-btn mr-1").attr("data-id", item
                                .id).attr("data-code", item.code).attr("data-name", item
                                .name).text("Print"));
                            actionButtons.append($("<button>").addClass(
                                    "btn btn-danger delete-btn").attr("data-id", item.id)
                                .text("Delete"));
